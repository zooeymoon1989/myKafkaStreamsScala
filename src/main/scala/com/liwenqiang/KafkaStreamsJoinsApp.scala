...
object KafkaStreamsJoinsApp {
  def main(args: Array[String]): Unit = {
    ...
    val consumed: Consumed[String, Purchase] = Consumed.`with`[String, Purchase](stringSerde, purchaseSerde)
    val produced: Produced[String, Purchase] = Produced.`with`[String, Purchase](stringSerde, purchaseSerde)
    ...
    val joinedStream: kstream.KStream[String, CorrelatedPurchase] = coffeeStream.join(
      electronicsStream
      , purchaseJoiner
      , twentyMinuteWindow,
